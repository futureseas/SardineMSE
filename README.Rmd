...
# Main Workflows
These files and functions were used to 1) run the MSE scenarios, 2) evaluate model diagnostics, and 3) extract performance metrics for publication. NOTE: All final model runs and output were processed with code on the server branch of the GitHub repository; some functionality may differ between branches.
...
mse <- grViz("digraph {
              ...
              recdev [label = 'Generate Recruitment Deviations\n ExploreRecruitIndices\n CalculatePDOindex\n CalculateSSTindex']
              devOut [label = 'recdevPDOclim2101.csv\n recdevPDOnoclim2120.csv\n recdevMICE2100.csv\n recdevSST2070.csv']
              mse [label = '*OM_*EM_<scenario>_MSE']
              hcr [label = 'MS_sar_<HCR>']
              smry [label = 'SS_summary_all()']
              out [label = 'Summaries in Run Folders']
              
              recdev -> devOut -> mse -> smry -> out
              hcr -> mse
        }")
...
mse %>%
    export_svg() %>%
    charToRaw %>% 
    rsvg_png(file = "svgMSE.png")

# include_graphics("svgMSE.svg")
```
...
diag <- grViz("digraph {
              ...
              output [label = 'PullOutput']
              outFxn [label = 'GetSumryOutput()']
              results [label = 'server<scenarios>_<HCRs>_*Results']
              smry [label = 'SummarizeSardineMSE']
              perfMet [label = 'CalcPerformance()']
              termTS [label = 'CalcTermTS()']
              
              outFxn -> output -> results -> smry
              perfMet -> smry
              termTS -> smry
        }")
...
postViz <- grViz("digraph {
                  ...
                  results [label = 'server<scenarios>_<HCRs>_*Results']
                  suppl [label = 'SardineMSE_SupplInfo.Rmd']
                  plots [label = 'ManuscriptPlots']
                  comps [label = 'compDiagPlots()']
                  calcs [label = 'CalcPerformance()\nCalcTermTS()\nCalcRelMetric()\nCalcRebuildTime()\nCalcRelErr()']
...
postViz %>%
    export_svg() %>%
    charToRaw %>% 
    rsvg_png(file = "svgPostViz.png")
```
![](C:/Users/r.wildermuth/Documents/FutureSeas/SardineMSE/svgPostViz.png)

# Additional Support Functions and Code
Run batch code for multiple scenarios or iterations while insuring recdevs are the same 
