...
MS_sar_hcr2_bavg = function(EM_out_dir = NULL, 
  ...
  EMts = EM_out$sprseries %>% dplyr::filter(Era=="TIME")
  
  #Instead of using the forecast Age 1+ biomass for the next year from the EM, which can be bias, use
  #average of last three years of catches, excluding the terminal year, which can also be bias
  #as in the EM starter file the minimum age to calculate summary biomass is set at 1, Bio_Smry.1 can be used
  nyrs=dim(EMts)[1]
  bio1 = mean(EMts$Bio_Smry.1[(nyrs-3):(nyrs-1)])
