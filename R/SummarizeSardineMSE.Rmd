...
#           file = file.path(mseDir, "serverRandSSTPDOCyclRec_allHCRs_results.RDS"))
...
metricsTbl %>% group_by(scenario, recScen, HCR) %>%
  summarize(Nnonconvrg = sum(nonconvg, na.rm = TRUE))
```
...
errCompare <- cnvrgTS %>% filter(Seas == 1, model_run != omName) %>%
                ...
                summarize(runRE = mean(errSmryBio)) #%>%
...
errCompare %>% #filter(HCR != "HCR3") %>%
  ggplot(aes(x = HCR, y = runRE, fill = plotGroup)) +
