...
# EMmodelPath <- "J:/Desiree/Sardine/SardineMSE/scenarioModels/start2001"
...
datfile <- SS_readdat(file = paste0(OMmodelPath, "/constGrowth/data.ss"), version = "3.30")
...
sample_struct_list <- list("LowSteepShortALKfixMidN_RandRecHCR2" = sample_struct)

# figure out the recruitment deviation input ---------------

# define scenario name
scenName <- c("LowSteepShortALKfixMidN_RandRecHCR2")
iters <- 3
...
# out2nofor <- run_SSMSE(scen_name_vec = scenName[1], # name of the scenario
...
#                  OM_in_dir_vec = file.path(OMmodelPath, "constGrowthMidSteep_OM"), #rep(OMmodelPath, times = length(scenName)), # OM files
#                  EM_name_vec = c("constGrowSelfTest"), # Can't have number in name for summary diagnostics to work
#                  EM_in_dir_vec = c(file.path(EMmodelPath, "constGrowthMidSteep_EM")),
#                  MS_vec = c("MS_sar_hcr2_nofor"),
#                  custom_MS_source = file.path(MSfxnPath, "MS_sar_hcr2_nofor.R"),
...
out2 <- run_SSMSE(scen_name_vec = scenName, # name of the scenario
                  out_dir_scen_vec = mseOutputPath, # directory in which to run the scenario
                  iter_vec = rep(iters, times = length(scenName)), # run with 5 iterations for now
                  OM_name_vec = NULL, # specify directories instead
                  OM_in_dir_vec = file.path(OMmodelPath, "constGrowth"), #rep(OMmodelPath, times = length(scenName)), # OM files
                  EM_name_vec = c("constGrowShortEM"), # Can't have number in name for summary diagnostics to work
                  EM_in_dir_vec = c(file.path(EMmodelPath, "constantGrowth")),
...
# out2bavg <- run_SSMSE(scen_name_vec = scenName[4], # name of the scenario
#                   out_dir_scen_vec = mseOutputPath, # directory in which to run the scenario
...
#                   EM_name_vec = c("constGrowSelfTest"), # Can't have number in name for summary diagnostics to work
#                   EM_in_dir_vec = c(file.path(EMmodelPath, "constGrowthMidSteep_EM")),
#                   MS_vec = c("MS_sar_hcr2_bavg"),
#                   custom_MS_source = file.path(MSfxnPath, "MS_sar_hcr2_bavg.R"),
#                   use_SS_boot_vec = TRUE, # use the SS bootstrap module for sampling
#                   nyrs_vec = nyrs,        # Years to project OM forward
#                   nyrs_assess_vec = 1, # Years between assessments
#                   future_om_list = rand_dev_list, 
...
# cat("\n \n")
# out2bavg
# # copied rec_devs from constGrowthSteepness0dot6_OM into constGrowth/par.ss
# out0and2LowSteep <- run_SSMSE(scen_name_vec = scenName[3:4], # name of the scenario
#                       out_dir_scen_vec = mseOutputPath, # directory in which to run the scenario
#                       iter_vec = rep(iters, times = length(scenName[3:4])), # run with 5 iterations for now
#                       OM_name_vec = NULL, # specify directories instead
#                       OM_in_dir_vec = file.path(OMmodelPath, "constGrowth"), #rep(OMmodelPath, times = length(scenName)), # OM files
#                       EM_name_vec = c(NA, 
#                                       "constGrowSelfTest"), # Can't have number in name for summary diagnostics to work
#                       EM_in_dir_vec = c(NA, 
#                                         file.path(EMmodelPath, "constGrowth_EM")),
#                       MS_vec = c("no_catch", 
#                                  "MS_sar_hcr2"),
#                       custom_MS_source = file.path(MSfxnPath, "MS_sar_hcr2.R"),
#                       use_SS_boot_vec = TRUE, # use the SS bootstrap module for sampling
...
#                       future_om_list = rand_dev_list, 
#                       run_parallel = TRUE, # Run iterations in parallel
#                       sample_struct_list = sample_struct_list[3:4], # How to sample data for running the EM.
#                       seed = seedNum) #Set a fixed integer seed that allows replication
# cat("\n \n")
# out0and2LowSteep
# out3 <- run_SSMSE(scen_name_vec = scenName[3], # name of the scenario
#                   out_dir_scen_vec = mseOutputPath, # directory in which to run the scenario
...
#                   EM_name_vec = "constGrowSelfTest", # Can't have number in name for summary diagnostics to work
#                   EM_in_dir_vec = OMmodelPath,
#                   MS_vec = "MS_sar_hcr3", 
...
# out3
# 
# out5 <- run_SSMSE(scen_name_vec = scenName[4], # name of the scenario
#                       out_dir_scen_vec = mseOutputPath, # directory in which to run the scenario
#                       iter_vec = rep(iters, times = length(scenName[4])), # run with 5 iterations for now
#                       OM_name_vec = NULL, # specify directories instead
#                       OM_in_dir_vec = OMmodelPath, #rep(OMmodelPath, times = length(scenName)), # OM files
...
# out5
# 
# out6 <- run_SSMSE(scen_name_vec = scenName[5], # name of the scenario
#                   out_dir_scen_vec = mseOutputPath, # directory in which to run the scenario
