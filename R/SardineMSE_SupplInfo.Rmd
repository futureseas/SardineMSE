...
```
Figure S1: Fit of OM Stock Synthesis model (solid line) to acoustic-trawl survey data in calibration period.
```{r, kuriyamaComare}
mseOM$timeseries %>% select(Yr, Seas, Bio_smry) %>%
  ...
  facet_wrap(~Seas) +
  theme_minimal() +
  labs(caption = "Figure S2: Age 1+ biomass in the calibration period for the OM compared to estimated biomass\n from the Kuriyama et al. (2020) stock assessment for each semester (S1: left, S2: right).",
...
```{r, bioSeries, echo=FALSE, warning=FALSE, fig.height=8}
...
```{r, recSeries, echo=FALSE, warning=FALSE, fig.height=8}
...
```{r, catSeries, echo=FALSE, warning=FALSE, fig.height=8}
termTS %>% filter(model_run == omName) %>%
...
```{r, B0Series, echo=FALSE, warning=FALSE, fig.height=8, eval = FALSE}
...
kable(x = confErr %>% ungroup() %>%
            select(nAssess, nameHCR, recScen, closeTruePos, closeFalsePos, closeTrueNeg, closeFalseNeg, 
...
kable(x = confErr %>% ungroup() %>%
            select(nAssess, nameHCR, recScen, collapseTruePos, collapseFalsePos, collapseTrueNeg, collapseFalseNeg, 
