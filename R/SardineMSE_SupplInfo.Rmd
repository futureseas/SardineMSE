...
termTS %>% filter(model_run == omName, HCR == "HCR0") %>%
  ...
  caption = "Median (solid line) and individual trajectories (grey lines) under a No Catch (HCR0) management rule\n for each recruitment scenario.") +
...
termTS %>% filter(model_run == omName) %>%
  ...
  caption = "Median (solid line) and individual trajectories (grey lines) under each HCR\n for each recruitment scenario.") +
  scale_linetype_manual(values = rep("solid", 500)) +
  # scale_color_manual(values = brewer.pal(n = 5, "Pastel1")) +
...
annualRelBioCat %>% filter(model_run == omName, year > 2019) %>%
  ...
  labs(y = "Catch:Mean", x = "Age1+ Biomass:Mean",
       ...
       caption = "Interaction between annual catch and annual Age1+ biomass under each HCR (colored points) in each recruitment scenario.") +
...
```{r, metricStats}
metricsTbl %>% full_join(y = annualRelBioCat, 
                         by = c("model_run", "iteration", "scenario", 
                                "HCR", "recScen", "nameHCR")) %>%
...
confErr %>% select(nAssess, closeTruePos, closeFalsePos, closeTrueNeg, closeFalseNeg, 
...
confErr %>% select(nAssess, collapseTruePos, collapseFalsePos, collapseTrueNeg, collapseFalseNeg, 
