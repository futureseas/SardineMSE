# Code to self test a constant growth model starting in 2001 with perfect information 
# Created: 4/20/2022, Robert Wildermuth
# Details: Uses a constant growth operating model beginning in 2001 with high 
#          sample precision to test issues with EM convergence 
...
OMFixed <- "C:/Users/r.wildermuth/Documents/FutureSeas/SardineMSE/scenarioModels/start2001/fixedParams"
OMRegime <- "C:/Users/r.wildermuth/Documents/FutureSeas/SardineMSE/scenarioModels/start2001/fixedParamsRegime"
OMR0 <- "C:/Users/r.wildermuth/Documents/FutureSeas/SardineMSE/scenarioModels/start2001/fixedParamsR0"
OMInitF <- "C:/Users/r.wildermuth/Documents/FutureSeas/SardineMSE/scenarioModels/start2001/fixedParamsInitF"
OMGrowth <- "C:/Users/r.wildermuth/Documents/FutureSeas/SardineMSE/scenarioModels/start2001/fixedParamsGrowth"
...
datfile <- SS_readdat(file = paste0(OMFixed, "/data.ss"), version = "3.30")
...
lencomp <- data.frame(Yr = rep(c(yrsrt:yrend),nldat), 
                      ...
                      Nsamp = c(rep(2000,nyrs),rep(2000,nyrs),rep(2000,nyrs),rep(2000,nyrs)))
...
sample_struct_list <- list("fixedParams2001OM_selfTest_RandRecHCR2" = sample_struct,
                           "fixedParams2001OM_selfTestRegime_RandRecHCR2" = sample_struct,
                           "fixedParams2001OM_selfTestR0_RandRecHCR2" = sample_struct,
                           "fixedParams2001OM_selfTestInitF_RandRecHCR2" = sample_struct,
                           "fixedParams2001OM_selfTestGrowth_RandRecHCR2" = sample_struct)

# figure out the recruitment deviation input ---------------

# define scenario name
scenName <- c("fixedParams2001OM_selfTest_RandRecHCR2",
...
outFixedAll <- run_SSMSE(scen_name_vec = scenName[1], # name of the scenario
                 ...
                 OM_in_dir_vec = OMFixed, #rep(OMmodelPath, times = length(scenName)), # OM files
                 EM_name_vec = c("fixedParamsSelfTest"), # Can't have number in name for summary diagnostics to work
                 EM_in_dir_vec = c(OMFixed),
                 MS_vec = c("MS_sar_hcr2"),
...
outRegime <- run_SSMSE(scen_name_vec = scenName[2], # name of the scenario
                  out_dir_scen_vec = mseOutputPath, # directory in which to run the scenario
                  iter_vec = rep(iters, times = length(scenName[2])), # run with 5 iterations for now
                  OM_name_vec = NULL, # specify directories instead
                  OM_in_dir_vec = OMRegime, #rep(OMmodelPath, times = length(scenName)), # OM files
                  EM_name_vec = "fixedParamsSelfTest", # Can't have number in name for summary diagnostics to work
                  EM_in_dir_vec = OMRegime,
                  MS_vec = "MS_sar_hcr2", 
                  custom_MS_source = file.path(MSfxnPath, "MS_sar_hcr2.R"),
                  use_SS_boot_vec = TRUE, # use the SS bootstrap module for sampling
                  nyrs_vec = nyrs,        # Years to project OM forward
...
outR0 <- run_SSMSE(scen_name_vec = scenName[3], # name of the scenario
                       out_dir_scen_vec = mseOutputPath, # directory in which to run the scenario
                       iter_vec = rep(iters, times = length(scenName[3])), # run with 5 iterations for now
                       OM_name_vec = NULL, # specify directories instead
                       OM_in_dir_vec = OMR0, #rep(OMmodelPath, times = length(scenName)), # OM files
                       EM_name_vec = "fixedParamsSelfTest", # Can't have number in name for summary diagnostics to work
                       EM_in_dir_vec = OMR0,
                       MS_vec = "MS_sar_hcr2", 
                       custom_MS_source = file.path(MSfxnPath, "MS_sar_hcr2.R"),
...
outInitF <- run_SSMSE(scen_name_vec = scenName[4], # name of the scenario
                      out_dir_scen_vec = mseOutputPath, # directory in which to run the scenario
                      iter_vec = rep(iters, times = length(scenName[4])), # run with 5 iterations for now
                      OM_name_vec = NULL, # specify directories instead
                      OM_in_dir_vec = OMInitF, #rep(OMmodelPath, times = length(scenName)), # OM files
                      EM_name_vec = "fixedParamsSelfTest", # Can't have number in name for summary diagnostics to work
                      EM_in_dir_vec = OMInitF,
                      MS_vec = "MS_sar_hcr2", 
                      custom_MS_source = file.path(MSfxnPath, "MS_sar_hcr2.R"),
                      use_SS_boot_vec = TRUE, # use the SS bootstrap module for sampling
                      nyrs_vec = nyrs,        # Years to project OM forward
...
outGrowth <- run_SSMSE(scen_name_vec = scenName[5], # name of the scenario
                  out_dir_scen_vec = mseOutputPath, # directory in which to run the scenario
                  iter_vec = rep(iters, times = length(scenName[5])), # run with 5 iterations for now
                  OM_name_vec = NULL, # specify directories instead
                  OM_in_dir_vec = OMGrowth, #rep(OMmodelPath, times = length(scenName)), # OM files
                  EM_name_vec = "fixedParamsSelfTest", # Can't have number in name for summary diagnostics to work
                  EM_in_dir_vec = OMGrowth,
                  MS_vec = "MS_sar_hcr2", 
                  custom_MS_source = file.path(MSfxnPath, "MS_sar_hcr2.R"),
                  use_SS_boot_vec = TRUE, # use the SS bootstrap module for sampling
                  nyrs_vec = nyrs,        # Years to project OM forward
